...
function computeEngine(candles, cfg){
  ...
  return {
    n, closes, highs, lows, hasVolume, maxScoreRef, atr: effAtr,
...
function renderAll(res, candles){
  ...
  // AI Insight & Self-Learning — dihitung ulang dari hasil engine yang sesungguhnya
  updateAIUI(calculateAIInsight(res));
  updateSelfLearningUI(res.stats);
...
// Dihitung dari hasil computeEngine() yang sesungguhnya (res), bukan data acak —
// dipanggil ulang setiap siklus render lewat renderAll() supaya card "AI Insight
// Engine" benar-benar mencerminkan kondisi pasar saat ini, bukan placeholder statis.
function calculateAIInsight(res) {

    const i = res.last;
    const prevI = Math.max(0, i - 1);

    const close = res.closes[i];
    const prevClose = res.closes[prevI];
    const atr = res.atr[i];
    const emaUp = res.emaTrendUp[i]; // true/false/null (null = belum cukup data utk EMA100)
    const tqiNow = res.tqi[i];

    let bias = emaUp === null ? "NEUTRAL" : (emaUp ? "BULLISH" : "BEARISH");
    let risk = "LOW";
    let regime = tqiNow > 0.6 ? "TRENDING" : tqiNow > 0.35 ? "NETRAL" : "CHOPPY";

    let confidence = 50 + Math.round(tqiNow * 30);

    // --- Momentum check
    const momentum = close - prevClose;
    if (Math.abs(momentum) > atr * 0.5) {
        confidence += 10;
    } else {
        risk = "HIGH NOISE";
        confidence -= 10;
    ...
    let insight;
    if (bias === "BULLISH") {
        insight = "Harga berada di atas EMA 100 dengan TQI " + tqiNow.toFixed(2) + ". Potensi continuation trend naik selama band SuperTrend belum tertembus.";
    } else if (bias === "BEARISH") {
        insight = "Harga berada di bawah EMA 100 dengan TQI " + tqiNow.toFixed(2) + ". Tekanan bearish dominan, waspadai pembalikan jika TQI menurun.";
    } else {
        insight = "Data candle belum cukup untuk EMA100 (butuh 100+ candle). Bias belum bisa ditentukan.";
    }

    return { bias, risk, regime, confidence, insight };
}
...
function updateAIUI(ai) {
    ...
    document.getElementById("aiBias").innerText =
        "Bias: " + ai.bias;

    document.getElementById("aiRisk").innerText =
        "Risk: " + ai.risk;

    document.getElementById("aiRegime").innerText =
        "Regime: " + ai.regime;
}

// Card "Self-Learning Engine" — aggressiveness/sensitivity dihitung dari win rate &
// avg-R riil (res.stats), bukan angka statis 1.00/1.00 yang tidak pernah berubah.
let learnState = { aggression: 1.0, sensitivity: 1.0 };
function updateSelfLearningUI(stats) {
    if (stats.winRate != null) {
        const delta = stats.winRate > 65 ? 0.03 : stats.winRate < 45 ? -0.03 : 0;
        learnState.aggression = clamp(learnState.aggression + delta, 0.5, 1.5);
    }
    if (stats.avgR != null) {
        learnState.sensitivity = clamp(1 + stats.avgR * 0.1, 0.5, 1.5);
    }
    const elAggro = document.getElementById('aiAggro');
    const elSense = document.getElementById('aiSense');
    const elTime = document.getElementById('aiLearnTime');
    if (elAggro) elAggro.textContent = learnState.aggression.toFixed(2);
    if (elSense) elSense.textContent = learnState.sensitivity.toFixed(2);
    if (elTime) elTime.textContent = new Date().toLocaleTimeString('id-ID');
}

function buildGPTPrompt(state) {

    return `
You are a professional XAUUSD trading analyst.

Analyze current market based on SATS engine data:
...
2. Trade bias (BUY/SELL/WAIT)
3. Risk warning
4. Short strategy suggestion

Keep it concise and professional.
`;
}

async function runGPTAnalysis(state) {

    const prompt = buildGPTPrompt(state);

    // ===== FALLBACK MODE (tanpa API) =====
...
- Avoid chasing move
- Focus on EMA100 alignment
        `;
    }

    // ===== REAL API MODE (optional future) =====
...
document.getElementById("gptRunBtn").addEventListener("click", async () => {

    if (!lastResult) {
        document.getElementById("gptOutput").innerText = "Mulai engine dulu (tombol ▶ Mulai) sebelum generate analisis.";
        return;
    }

    const i = lastResult.last;
    const emaUp = lastResult.emaTrendUp[i];
    const gptState = {
        price: fmtPrice(lastResult.closes[i]),
        trend: lastResult.stTrend[i] === 1 ? "BULLISH" : "BEARISH",
        ema: emaUp === null ? "N/A (data belum cukup)" : (emaUp ? "harga di atas EMA100" : "harga di bawah EMA100"),
        tqi: lastResult.tqi[i].toFixed(2),
        bias: lastResult.stTrend[i] === 1 ? "BULLISH" : "BEARISH",
        risk: lastResult.tqi[i] < 0.35 ? "HIGH (choppy)" : "NORMAL",
    };

    document.getElementById("gptOutput").innerText = "Menganalisis…";
    const output = await runGPTAnalysis(gptState);

    document.getElementById("gptOutput").innerText = output;
});

/* ═══════════════════════════════════════════════════════════
   Selesai. Semua kode di bawah titik ini pada versi sebelumnya
   (RT_ENGINE, WS_ENGINE, MTF, quant/sentiment engine, backtest,
   strategy lab, dashboard v21–v27, geopolitical/fund core,
   multi-agent, self-aware/organism system, dst.) dihapus karena
   tidak pernah dipanggil di alur eksekusi manapun — kode mati
   yang tidak memengaruhi tampilan atau fungsi aplikasi.
   ═══════════════════════════════════════════════════════════ */
